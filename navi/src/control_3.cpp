...
private:
    ...
    ros::Subscriber visual_cmd;
    ros::Subscriber visual_finish_flag;
    ros::NodeHandle nh;
    float yaw_x;
    float yaw_y;
    float yaw_x_;
    ...
    float visual_linear_x =0;
    float visual_linear_y =0;
    float visual_angular_z =0;
    bool visual_mode = false;
    bool visual_finish = false;
public:
    Control()
    { 
        ...
        visual_cmd = nh.subscribe("/visual_cmd_vel", 1, &Control::visual_callback, this);
        visual_finish_flag = nh.subscribe("/mission3_state", 1, &Control::visual_finish_callback, this);

    }
    void visual_finish_callback(const std_msgs::Int32MultiArray& msg){
        if (msg.data[0]==0){
            visual_finish = true;
        }
    }
    void visual_callback(const geometry_msgs::Twist& msg){
        visual_linear_x = msg.linear.x;
        visual_linear_y = msg.linear.y;
        visual_angular_z = msg.angular.z;
        visual_mode = true;
    }
    ...
    void ekf_odom_callback(const nav_msgs::Odometry& msg){
        ...
        if (outdoor_srv_running == true){
            if (visual_mode == false){
                ...
                vel.linear.x = visual_linear_x;
                vel.linear.y = visual_linear_y;
                vel.angular.z = visual_angular_z;
                pub_cmd.publish(vel);
            }
        }
