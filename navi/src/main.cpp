...
ros::Publisher pub_ekf_odom_estimation;
...
void EKF(){
        ...
        mat_init_with_GPS = Mat::zeros(3, 1, CV_64FC1);
        if (init_with_GPS == true){
            mat_init_with_GPS.ptr<double>(0)[0] = m_gps_data.pos_x;
            mat_init_with_GPS.ptr<double>(1)[0] = m_gps_data.pos_y;
            mat_init_with_GPS.ptr<double>(2)[0] = h_gps;
            m_ekf.m_xhat = mat_init_with_GPS.clone();
        }
        init_with_GPS = false;

        m_ekf.EKF_Predictionstep(m_ekf.m_xhat, m_ekf.m_Phat, vel_and_yawRate, interval);
        if (gps_flag == true){
            if (heading_update ==true && pose_update == true){
                m_ekf.EKF_Correctionstep(m_ekf.m_xhat, m_ekf.m_Phat, m_gps_data.quality_indicator, true, true, GPS_data); //check //true
            }
            else if (heading_update ==true && pose_update == false){
                m_ekf.EKF_Correctionstep(m_ekf.m_xhat, m_ekf.m_Phat, m_gps_data.quality_indicator, false, true, GPS_data); //check //true
            }
            else if (heading_update ==false && pose_update == true){
                m_ekf.EKF_Correctionstep(m_ekf.m_xhat, m_ekf.m_Phat, m_gps_data.quality_indicator, true, false, GPS_data); //check //true
            }
            else if (heading_update ==false && pose_update == false){
                // m_ekf.EKF_Correctionstep(m_ekf.m_xhat, m_ekf.m_Phat, m_gps_data.quality_indicator, false, false, GPS_data); //check //true
            }
        }
        ...
        m_d_nav_result[2] = m_ekf.m_xhat.ptr<double>(2)[0]*180.0/M_PI;
        m_d_nav_result[3] = m_gps_data.pos_x;
        m_d_nav_result[4] = m_gps_data.pos_y;
        m_d_nav_result[5] = h_gps*180.0/M_PI;
...
void subscribeCallback_utm(const nav_msgs::OdometryConstPtr& msg)
{
    ...
    double heading_update_thres = 30 * M_PI / 180;

    // if ((fabs(m_dGPS_UTM_Heading - m_dGPS_UTM_Heading_prev) > heading_update_thres)||(m_odom_data.velocity < update_threshold_velocity)){
    //     heading_update = false;
    // }
    // else{
    //     heading_update = true;
    // }
    if ((fabs(m_odom_data.velocity) < update_threshold_velocity)&&(fabs(vel_y)< update_threshold_velocity)){
        pose_update = false;
    }
    else{
        pose_update = true;
    }
    if (fabs(vel_ave)>heading_update_threshold_speed_x && vel_y <heading_update_threshold_speed_y && c_yaw_rate < heading_update_threshold_angular_z){
        heading_update = true;
    }
    else{
        heading_update = false;
...
void subscribeCallback_init(const std_msgs::Bool::ConstPtr& msg)
{
    init_with_GPS = msg->data;
}
int main(int argc, char** argv)
{
    ...
    ros::NodeHandle nh;
    ros::Publisher pub_pose_estimation = nh.advertise<std_msgs::Float64MultiArray>("POS_T",1);
    pub_ekf_odom_estimation = nh.advertise<nav_msgs::Odometry>("odometry/EKF_estimated",1);
    ros::Subscriber listener_imu = nh.subscribe("/gx5/imu/data",10, subscribeCallback_imu, ros::TransportHints().tcpNoDelay());
