...
class URScriptControl:
    ...
    def create_ur_script_message(self, speed_x, speed_y):
        ur_script = String()
        #if abs(speed_x) <= 0.0001 and abs(speed_y) <= 0.0001:
        #    ur_script.data = "stopl(0.1)"
        #else:
    ...
    def box_callback(self, data):
        ...
        rospy.loginfo('speed:{} {}'.format(self.speed_x, self.speed_y))

        self.ur_script_pub.publish(self.create_ur_script_message(self.speed_x, self.speed_y))
        self.pre_speed_x = self.speed_x
        self.pre_speed_y = self.speed_y
