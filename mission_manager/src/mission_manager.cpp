...
int main(int argc, char** argv){
  ...
  int orange_load_cnt = 0;
  int blue_load_cnt = 0;
  int green_load_cnt = 0;
  int red_load_cnt = 0;

  bool orange_load_done_flg = false;
  ...
  for(int cur_mission_idx = 0; cur_mission_idx < mission_seq.size();) {
    ...
    switch(cur_mission) {
      ...
      case MISSION_LOAD_ORANGE_BRICKS:
      {
        ...
        //======================= for loop 5 for right container
        mission_preset.set_ur_action(UR_ACTION_LOAD);
        mission_preset.set_ur_target_brick_color(COLOR_ORANGE);
        ...
        if(orange_load_cnt >= 10) {
          orange_load_done_flg = true;
          cur_mission_idx++;
        }
      ...
      case MISSION_UNLOAD_ORANGE_BRICKS:
      {
        ...
        if(unload_left_done_flg && unload_right_done_flg == true) {
          ...
          orange_load_done_flg = !orange_unload_done_flg;
          orange_load_cnt = 0;
          cur_mission_idx++;
        }
      ...
      case MISSION_LOAD_MULTICOLOR_BRICKS:
      {
        ...
        for(int left_layer_cnt = 0; left_layer_cnt < re_arraged_left.size(); ) 
        {
          for(int layer_ele_cnt = 0; layer_ele_cnt < re_arraged_left[left_layer_cnt].size(); ) 
          {
            mission_preset.set_ur_target_brick_color(re_arraged_left[left_layer_cnt][layer_ele_cnt]);
        ...
        for(int right_layer_cnt = 0; right_layer_cnt < re_arraged_left.size(); ) 
        {
          for(int layer_ele_cnt = 0; layer_ele_cnt < re_arraged_left[right_layer_cnt].size(); ) 
          {
            mission_preset.set_ur_target_brick_color(re_arraged_left[right_layer_cnt][layer_ele_cnt]);
