...
class GotoBrick:
    def __init__(self):
        ...
        self.publish_finish              = rospy.get_param('~publish_finish', False)
        ...
        if self.service_control:
            ...
            rospy.spin()
        if self.publish_finish:
            self.finish_pub = rospy.Publisher('/arrived', Bool, queue_size=10)
    ...
    def calc_twist(self):
        ...
        if self.is_arrived():
            ...
            control_speed.linear.y = self.raw_y * self.y_axis_reduce_gain
            sum_quaternion = pose_msg.pose.orientation.x + pose_msg.pose.orientation.y + pose_msg.pose.orientation.z + pose_msg.pose.orientation.w
    ...
    def is_arrived(self):
        #if -0.001 < self.raw_x < 0.5 and -0.4 < self.raw_y < 0.4: #brick condition
        if -0.001 < self.raw_x < 0.5 and -0.05 < self.raw_y < 0.05: #segmentation condition
            return True
        else:
            return False
