...
# Global variable
servoing_flag = False # for checking (waypoint tracking / visual servoing mode)
...
class GotoBrick:
    def __init__(self):
        ...
        # Subscribe
        self.waypoint_flag = rospy.Subscriber('/waypoint_tracking', Bool, self.callback_waypoint)
    ...
    def callback_waypoint(self, mission_msg):
        global servoing_flag
        if mission_msg.data == True:
            servoing_flag = False
        else:
            servoing_flag = True
    ...
    def run(self, req=None):
        ...
        while not rospy.is_shutdown():
            ...
            # Visual sesrvoing only when servoing flag is true (Not waypoint tracking mode)
            if servoing_flag == True:
                self.calc_twist()
                self.publish_twist()
