...
class JointPublisher:
    def __init__(self):
        ...
        self.front_pub = rospy.Publisher('/front/odrive/odrive_joint', 
                JointTrajectoryControllerState, queue_size=2)
        self.back_pub = rospy.Publisher('/back/odrive/odrive_joint', 
                JointTrajectoryControllerState, queue_size=2)
        rospy.Subscriber('/joint_command', JointTrajectoryControllerState, self.joint_callback)
        rospy.spin()
    ...
    def turn_dynamixel(self, motor_id, angle):
        ...
        try:
            turn_motor = rospy.ServiceProxy('dynamixel_workbench/dynamixel_command', DynamixelCommand)
            value = self.radian2value(angle)
            resp1 = turn_motor('', motor_id, 'Goal_Position', value)
